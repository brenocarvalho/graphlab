        ...
        **/
        void normalize(){
            ...
            if(scope.size() == 1){
                ...
                std::cout << "normalizing many vars" << std::endl;
                for(cpd_type::const_iterator iter = cpd.begin(); iter != cpd.end(); iter++){
                    std::cout <<" - "  << *iter;
                }
                std::cout << std::endl;
                for(size_t iter = 0; iter < sub_cpd_size; iter++){
                    ...
                    for(size_t first_var_iter = 0; first_var_iter < var_size; first_var_iter++){
                        ...
                        std::cout << cpd[0] << "updating z " << cpd[iter + sub_cpd_size*first_var_iter] << std::endl;
                    }
                    //For each assignment in the cpd which the first variable is equals to 'first_var_iter':
...
/*class EvidenceEngine:  public graphlab::ivertex_program<GraphLab_type, int>,
                       public graphlab::IS_POD_TYPE{
    private:
    ...
    public:
        ...
        void runInference(graphlab::distributed_control& dist_ctrl, GraphLab_type& dgraph){
            graphlab::omni_engine<EvidenceEngine> engine(dist_ctrl, dgraph, "sync");
            engine.signal_all();
            engine.start();
        }
...
class Learning_Complete_Data_Engine{
    private:
        ...
        static Factor& absorb_data(Factor& fact, DataSet& data){
            ...
            std::cout<< "FACTOR POS COUNTING " << fact << std::endl;
            fact.normalize();
            return fact;
        }
...
*/
    ...
    std::ofstream output;
    output.open("/home/breno/pgm_test", std::ios::out | std::ios::app);
    output << "Time Elapsed: " << (end-begin)/ CLOCKS_PER_SEC << "s.\n";
    output.close();
